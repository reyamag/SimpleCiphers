...
class Playfair(CipherInterface):
        ...
        while i < len(pText):
            ...
            if i+1 != len(pText):
                if pText[i] == pText[i+1]:
                    # Split the a same-letter pair by adding an 'x'
                ...
                # Add 'x' to single last character
                pTextPaired += pText[i] + 'x'
                i += 1
    ...
    def decrypt(self, cText):
        ...
        # Get ciphertext - same logic/comments as encrypt()
        i = 0
        while i <len(cText):
            pair = cText[i:i+2]
            ...
            if first[0] == second[0]:
                # Shift left/up instead of down/right
                newFirst = (first[0], (first[1]-1) % 5)
                newSecond = (second[0], (second[1]-1) % 5)
            elif first[1] == second[1]:
                newFirst = ((first[0]-1) % 5, first[1])
